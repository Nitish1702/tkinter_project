...
os.chdir('/Users/nitishkumar/Desktop/Proj1')
r=t.Tk()
r.title('bank system')
...
def check_value1(name):
    ...
    if  ent_phone1.get().isdigit() ==False or len(ent_phone1.get())!=10:
        ...
        messagebox.showinfo('Error','Please enter correct name value')
    try:
        if  (int(e_d1.get())>31 or int(e_d1.get())<1) or( int(e_m1.get())<1 or int(e_m1.get())>12) or len(e_y1.get())!=4 :
            messagebox.showinfo('Error','Please enter correct Date of birth  value')
        else:
            editing_acc(name)
    ...
    if e3==otp:
        if (name in op['list1']) :
            ...
            if o['password']==password:
                ...
                def sub_wid(name):
                    if e_withdraw.get().isdigit()== True:
                        ...
                        if detail['balance']>2000:
                            ...
                            k.append(str(detail['index'])+'    '+str(d.day)+ '/'+str(d.month)+'/'+str(d.year)+'                   ' + e_withdraw.get()+'dr.     '+str(detail['balance']))
                            detail['index']+=1
                            print(k)
                            print(str(detail['index'])+'    '+str(d.day)+ '/'+str(d.month)+'/'+str(d.year)+'                   ' + e_withdraw.get()+'dr.     '+str(detail['balance']),'     ****helloweoihfweenfkeiifif')
                ...
                def add_dep(name):
                    if e_deposit.get().isdigit()== True:
                        ...
                        tj.config(state='disabled')
                        k.append(str(detail['index'])+'    '+str(d.day)+ '/'+str(d.month)+'/'+str(d.year)+'     '+e_deposit.get()+'cr.     '+'              ' +str(detail['balance']))
                        print(k)
                        detail['statement']=k
                        detail['index']+=1
                            #print(detail['statement'])
            ...
            # scroll.place(x=460,y=230)
                ...
                b_edit=ttk.Button(s,text='Edit Profile',width=10,command=lambda:edit(frame,name)).place(x=10,y=40)
...
def send(x):
    ...
    s.starttls()
    s.login("utkarshsingh9839@gmail.com" , "8881411122 Utkarsh@#")
    otp = random.randint(1000, 9999)
    otp1 = str(otp)
        
    s.sendmail("utkarshsingh9839@gmail.com" , x , otp1)
...
def main():
    ...
    r.state('normal')
    global ent
    frame=LabelFrame(r,text='hello',padx=100,pady=60)
    ...
    def otp(name):
        ...
        confirmbut=Button(frame,text='LOGIN',command=lambda:open1(ent.get(),e2.get(),frame,e3.get(),otp1))
